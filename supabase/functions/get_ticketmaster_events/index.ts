...
Deno.serve(async (req) => {
  ...
  try {
    ...
    while (page < totalPages && page < 50) {
      ...
      const ticketmasterUrl = `https://app.ticketmaster.com/discovery/v2/events.json?apikey=${consumerKey}&countryCode=US&size=200&page=${page}`;

      console.log(`Fetching page ${page + 1} from Ticketmaster API`);
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Successfully fetched and stored ${insertedCount} events from Ticketmaster`,
